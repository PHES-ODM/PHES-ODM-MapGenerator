...
Mapping files are Excel files that contain all required configuration information for mapping from a source dataset (eg. NWSS) to a target dataset (eg. ODM v2). Each mapping file can have multiple tabs, one that defines some basic mappings from source to target slots, and one or more separate tabs configuring how wide columns are mapped. While the tabs in mapping configuration files are typically named `maps`, `wide`, and `enums`, they can be given any name, as long as the names are provided when running the appropriate scripts.
