...
are also read on GitHub and link to source files and to the root-level README
and CONTRIBUTING using repository-relative paths — targets that live outside
`docs_dir` and that mkdocs cannot resolve. mkdocs cannot tell those apart from
genuinely broken links, so it validates neither.

This script does, by resolving every relative link against the filesystem
instead of against mkdocs' page list. It also checks that `#anchor` fragments
...
def anchor_slug(heading: str) -> str:
    ...
    why `### Target Value Columns (_value)` anchors as
