...
- *cleaned_data*: Contains all input data, from the source dataset, that has been cleaned from *input_data_dir*. Cleaning of data involves doing some minor cleanup, such as correcting capitalization and data types. If *input_data_dir* is specified then mapping will be performed on this cleaned data using the generate LinkML map schemas.
