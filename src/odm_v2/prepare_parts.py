...
from utils.general_utils import save_data_frame, get_logger, expand_multi_rows
...
def add_v1_enumeration_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _make_enumeration_name(row: pd.Series) -> str:
        ...
        if str(row[V2MappingColumns.SOURCE_LOCATION]).lower() == V2MappingVariableLocations.VARIABLE_CATEGORIES.lower():
        ...
        return None
    df = df.copy()
    df[V2MappingColumns.SOURCE_ENUM_NAME] = df[[V2MappingColumns.SOURCE_LOCATION, V2MappingColumns.SOURCE_TABLE, V2MappingColumns.SOURCE_VARIABLE]].apply(_make_enumeration_name, axis=1)
    return df

def prepare_parts(parts_file: str, output_file: str, map_columns: Optional[Dict[str, str]] = None):
    ...
    df = df[~pd.isna(df[V2MappingColumns.SOURCE_TABLE]) & ~(df[V2MappingColumns.SOURCE_TABLE].astype(str).str.lower() == NA_TAG.lower())]
    ...
    df = expand_multi_rows(df, columns = [V2MappingColumns.SOURCE_TABLE, V2MappingColumns.SOURCE_LOCATION, V2MappingColumns.SOURCE_VARIABLE, V2MappingColumns.SOURCE_CATEGORY])
    
    # For any value that has AND_TAG in it, select the first item (eg. for "conf & report" we will select "conf")
    df = select_and_values(df, columns = [V2MappingColumns.SOURCE_TABLE, V2MappingColumns.SOURCE_LOCATION, V2MappingColumns.SOURCE_VARIABLE, V2MappingColumns.SOURCE_CATEGORY])
...
if __name__ == "__main__":
    if "get_ipython" in globals():
        ...
        args.add_argument("--output_file", type=str, help="CSV file to save the prepared file to", required=True)
    ...
    prepare_parts(opts.parts_file, opts.output_file, map_columns=map_columns)
