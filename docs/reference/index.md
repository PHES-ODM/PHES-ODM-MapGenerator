...
| [CLI Configuration Files](cli-config-files.md) | Every `make_mappers_cli.py` command-line option, the YAML `--config` file format, path resolution, and slot format operations |
...
The generator's own options are listed under
[Command-line options](cli-config-files.md#command-line-options), and printed by:
