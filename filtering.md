...
Filtering is performed using boolean filters that are given names and that are applied to various classes (ie. DataFrames). The names given to the filters in the example configuration table above are referenced in the `inputFilter` and `outputFilter` columns. The names can be any user-defined string. The first time a filter is created it consists of all `True` values, meaning it will select all rows in the DataFrame. It will iteratively me modified with each row in the configuration, and when complete it can be applied to a class (ie. a DataFrame).
...
Copy the class (DataFrame) in `class` and save it as the name in `value`. If a class with the same name already exists then it is overwritten. This will not apply any existing filter to the DataFrame while copying. If a filter exists for the `class` then `apply_filter` must be performed using the filter before `copy_class`. After copying the DataFrame it can be used in any subsequent row of the configuration.
...
Modify the filter in `inputFilter` to drop rows in class `class` where the value in `slot` is a duplicate. If duplicates are found then only the first duplicate in the set is retained and the others are dropped. The resulting filter is saved in `outputFilter`.

### drop_duplicates_keep_last

Modify the filter in `inputFilter` to drop rows in class `class` where the value in `slot` is a duplicate. If duplicates are found then only the last duplicate in the set is retained and the others are dropped. The resulting filter is saved in `outputFilter`.

### delete_filter

Delete the filter in `inputFilter`.

### exclude_equals

Modify the filter in `inputFilter` to exclude any row that has a value found in the `value` column of the configuration. It allows multiple values as arrays, such as `["", -1]` (which will exclude rows where the slot is blank or -1). The resulting filter will be saved with the name in `outputFilter`.
