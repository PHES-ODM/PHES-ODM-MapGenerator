...
def clean_data_file(
    ...
    if output_dir == os.path.dirname(file):
        raise ValueError(
            f"The output_dir and input file directory must be different. output_dir is '{output_dir}' and file is '{file}'"
        )
...
def clean_data_directory(
    ...
    for f in os.listdir(directory):
        if os.path.splitext(f)[1].lower() in [".tsv", ".txt", ".csv"]:
            output_file, df = clean_data_file(
                Path(directory) / f,
                schema=schema,
...
if __name__ == "__main__":
    if "get_ipython" in globals():
        ...
        args.add_argument(
            "--file",
            type=str,
            help="Clean this file. Must be a csv, txt, or tsv file. txt files are treated as tab separated",
            required=False,
        )
        args.add_argument(
            "--output_dir",
