...
def load_data(
    data_dir: Union[Path, str], recognized_classes: Optional[List[str]] = None
    ...
    for file in os.listdir(data_dir):
        ...
        df = read_data_frame(
            os.path.join(data_dir, file), keep_default_na=False, na_values=[""]
        )
        if file_class not in data:
            data[file_class] = df
        else:
            data[file_class] = pd.concat(
                [data[file_class], df], ignore_index=True
...
def run_filter(
    filter_config_file: Union[Path, str],
    *,
    ...
    # Load values as YAML
    config_df[FilterConfigColumns.VALUE] = config_df[FilterConfigColumns.VALUE].map(
        yaml.safe_load
    )
    ...
    if data is None:
        data = load_data(
            data_dir,
            recognized_classes=list(config_df[FilterConfigColumns.CLASS].unique()),
        )
    else:
...
if __name__ == "__main__":
    if "get_ipython" in globals():
        ...
        args.add_argument(
            "--data_dir",
            type=str,
            help="Location of the CSV or TSV data files to filter. The file names must be class names in the schema.",
            required=True,
        )
        args.add_argument(
            "--filter_config_file",
