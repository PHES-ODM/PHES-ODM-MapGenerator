...
class IDGenerator(object):
    ...
    def prepare_config(self, config: str):
        ...
        if self.config.get(ConfigKeys.CLASS_LINKAGES, None):
            for source_class, target_linkages in self.config[ConfigKeys.CLASS_LINKAGES].items():
                for target_class, linkages in target_linkages.items():
                    ...
                    for idx, linkage in enumerate(linkages):
                        ...
                        linkage[LinkageKeys.SOURCE_SLOT] = _make_orig(source_class, linkage[LinkageKeys.SOURCE_SLOT])
                        linkage[LinkageKeys.TARGET_SLOT] = _make_orig(target_class, linkage[LinkageKeys.TARGET_SLOT])
    ...
    def make_all_ids(self, class_names: Optional[Union[str, List[str]]]=None, row_indices: Optional[Union[int, List[int]]]=None):
        ...
        total_ids = np.sum([len(self.class_info[c][ClassInfoKeys.DATA]) * len(self.class_ids.get(c, [])) for c in class_names])
        processed_ids = 0
        
        for idx, class_name in enumerate(class_names):
            _log_info(f"Making IDs for class '{class_name}' ({idx+1}/{len(class_names)})")
            ...
            if row_indices is None:
                ...
                row_indices = range(0, len(self.class_info[class_name][ClassInfoKeys.DATA]))
            else:
            ...
            for idx in tqdm(row_indices) if output_progress else row_indices:
                ...
                for slot in all_slots:
                    ...
                    v = self.class_info[class_name][ClassInfoKeys.DATA][idx, self.get_column_index(class_name, slot)]
                    if not pd.isna(v):
                        continue
    ...
    def set_data_value(self, class_name: str, slot: str, row_index: int, v: Any):
        ...
        """
        self.class_info[class_name][ClassInfoKeys.DATA][row_index, self.get_column_index(class_name, slot)] = v
    ...
    def get_rows_at_index(self, class_name: str, index: Union[int, List[int]]) -> np.ndarray:
    ...
    def get_linked_rows(self, source_class: str, source_index: Union[int, List[int]], target_class: str, linkage_path: Optional[Union[Dict, List[Dict]]]=None, return_indices: Optional[bool]=False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        for linkage in linkage_path:
            ...
            values = list(np.unique(rows[:, self.get_column_index(source_class, linkage_source_slot)]))
            rows, indices = self.get_rows_equal(linkage_target_class, linkage_target_slot, values, return_indices=True)
            if rows is None:
    ...
    def save_all(self, output_dir: str, orig_columns_only: bool=True, drop_duplicates: bool=True):
        ...
        for class_name, info in self.class_info.items():
            ...
            if pk_slot:
                ...
                logger.info(f"Dropped duplicate primary keys for class '{class_name}': {orig_len} -> {new_len} ({new_len-orig_len})")
